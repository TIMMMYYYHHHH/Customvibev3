// Inline SVG icon set, replacing lucide-react. Same 24x24 stroke style
// (stroke=currentColor, strokeWidth 2, round caps/joins) as the icons this
// site used before the rewrite, so existing className-driven sizing/color
...
const PATHS = {
  ...
  HelpingHand: '<path d="m2 13 6 6"/><path d="M12.8 19.6A2 2 0 1 0 14 16H8.5l-2.387-2.745A3 3 0 0 0 4.27 12.3l-2.244-.359"/><path d="M13.5 16H21a2 2 0 0 0 0-4h-5.5l-2.4-3.6a2 2 0 0 0-1.667-.9H8a2 2 0 0 0-1.8 1.2l-.6 1.2"/><path d="m7 18 1.6-3.2"/>',
  ...
  MessageCircle: '<path d="M2.992 16.342a2 2 0 0 1 .094 1.167l-1.065 3.29a1 1 0 0 0 1.236 1.236l3.29-1.065a2 2 0 0 1 1.167.094 10 10 0 1 0-4.722-4.722"/>',
};

export function iconSvg(name, { size = 16, className = '', strokeWidth = 2, fill = 'none' } = {}) {
  const inner = PATHS[name] || '';
  const cls = className ? ` class="${className}"` : '';
  return `<svg${cls} width="${size}" height="${size}" viewBox="0 0 24 24" fill="${fill}" stroke="currentColor" stroke-width="${strokeWidth}" stroke-linecap="round" stroke-linejoin="round">${inner}</svg>`;
}
